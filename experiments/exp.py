...
if __name__ == "__main__":
    if not os.path.exists('simulation/RLC_Sim/data/train/RLC_all.csv'):
        print('Creating file with merged data')
        concatenate_csv_files('simulation/RLC_Sim/data/train', 'simulation/RLC_Sim/data/train/RLC_all.csv')
    
    if not os.path.exists('simulation/RLC_Sim/data/train/RLC_all_small.csv'):
        print('Creating file with merged data')
        concatenate_csv_files('simulation/RLC_Sim/data/train', 'simulation/RLC_Sim/data/train/RLC_all_small.csv', small=True)
    
    if not os.path.exists('simulation/RLC_Sim/data/train/RLC_all_holes.csv'):
        print('Creating file with merged data')
        concatenate_csv_files('simulation/RLC_Sim/data/train', 'simulation/RLC_Sim/data/train/RLC_all_holes.csv', hole=True)

    # Get Min Max Values for global scaling
    folder_path = 'simulation/RLC_Sim/data/train'
    ...
    for var_model, torch_seed in hparam_combinations:
        ...
        train_file = f'simulation/RLC_Sim/data/train/RLC_{var_model}.csv'
        ...
        if test and not fast_dev_run:
            ...
            if not os.path.exists("./filelogs/"):
                ...
                os.makedirs("./filelogs/")
                print(f"Folder ./filelogs/ created.")
            else:
                print(f"Folder ./filelogs/ already exists.")

            # Model testing
            autoencoder.eval()
            with torch.no_grad():
                for test_case in test_cases:
                    logdir = f"./filelogs/{var_model}_{test_case}"

                    # Test case normal system
                    test_ok_file = f'simulation/RLC_Sim/data/test/RLC_Test_{test_case}.csv'
                    ...
                    test_cap_file = f'simulation/RLC_Sim/data/test/RLC_capacitance_{test_case}.csv'
                    ...
                    test_ind_file = f'simulation/RLC_Sim/data/test/RLC_inductance_{test_case}.csv'
                    ...
                    test_res_file = f'simulation/RLC_Sim/data/test/RLC_resistance_{test_case}.csv'
