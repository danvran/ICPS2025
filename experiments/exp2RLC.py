...
if __name__ == "__main__":
    if not os.path.exists('simulation/2RLC_Sim/data/train/2RLC_all.csv'):
        print('Creating file with merged data')
        concatenate_csv_files('simulation/2RLC_Sim/data/train', 'simulation/2RLC_Sim/data/train/2RLC_all.csv')
    
    if not os.path.exists('simulation/2RLC_Sim/data/train/2RLC_all_small.csv'):
        print('Creating file with merged data')
        concatenate_csv_files('simulation/2RLC_Sim/data/train', 'simulation/2RLC_Sim/data/train/2RLC_all_small.csv', small=True)
    
    if not os.path.exists('simulation/2RLC_Sim/data/train/2RLC_all_holes.csv'):
        print('Creating file with merged data')
        concatenate_csv_files('simulation/2RLC_Sim/data/train', 'simulation/2RLC_Sim/data/train/2RLC_all_holes.csv', hole=True)

    # Get Min Max Values for global scaling
    folder_path = 'simulation/2RLC_Sim/data/train'
    ...
    for var_model, torch_seed in hparam_combinations:
        ...
        csv_logger = CSVLogger('2RLC_logs', name=f'my_model_{var_model}_{torch_seed}')
        #exp_results = [var_model, None, None, None, None, None, None, None, None, None]
        train_file = f'simulation/2RLC_Sim/data/train/2RLC_{var_model}.csv'
        ...
        if test and not fast_dev_run:
            ...
            if not os.path.exists("./2RLC_filelogs/"):
                # Create the folder if it doesn't exist
                os.makedirs("./2RLC_filelogs/")
                print(f"Folder ./2RLC_filelogs/ created.")
            else:
                print(f"Folder ./2RLC_filelogs/ already exists.")

            # Model testing
            autoencoder.eval()
            with torch.no_grad():
                for test_case in test_cases:
                    logdir = f"./2RLC_filelogs/{var_model}_{test_case}"

                    # Test case normal system
                    test_ok_file = f'simulation/2RLC_Sim/data/test/2RLC_Test_{test_case}.csv'
                    ...
                    test_cap_file = f'simulation/2RLC_Sim/data/test/2RLC_capacitance_{test_case}.csv'
                    ...
                    test_ind_file = f'simulation/2RLC_Sim/data/test/2RLC_inductance_{test_case}.csv'
                    ...
                    test_res_file = f'simulation/2RLC_Sim/data/test/2RLC_resistance_{test_case}.csv'
