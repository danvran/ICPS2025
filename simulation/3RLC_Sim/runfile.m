...
if anomalies == true  % This must run after the "variables" script
    ...
    for f = 1:length(anomalyFiles)
        if anomalyFiles(f).isdir == 0 % Assure it's not a directory
            ...
            for i = 0:2:20
                ...
                csvname = sprintf('./data/test/3RLC_%s_%d.csv', clean_name, i);
    ...
    if test == true
        ...
        for i = 0:2:20
            ...
            sine_offset.Value = i;
            % Call the function with updated sine_offset
            csvname = sprintf('./data/test/3RLC_Test_%d.csv', i);
            processFile(csvname, anomalies, test)
        end
        ...
        for i = 0:2:20
            sine_offset.Value = i;
            csvname = sprintf('./data/train/3RLC_%d.csv', i);
            processFile(csvname, anomalies, test)
        end
    end
end
